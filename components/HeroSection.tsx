import Image from "next/image";
import Button from "./Button";


const HeroSection = () => {
  return (
    <section className='max-container  padding-container flex flex-col gap-20 py-10 pb-32 md:gap-28 lg:py-20 xl:flex-row border-2 '>
        <div className='hero-map '/>
      {/* left section*/}

      <div className="relative z-20 flex flex-1 flex-col xl:w-1/2">
        <Image src='/camp.svg' alt="camp" width={50} height={50} className="absolute  left-[-5px] top-[-30px] w-10 lg:w-[50px]"  />
        <h1 className="bold-52 lg:bold-88">Putuk Truno Camp Area</h1>
        <p className="regular-16 mt-6 text-gray-30 xl:max-w-[520px]">We want to be on each of your journeys seeking the satisfaction of seeing the incorruptible beauty of nature. We can help you on an adventure around the world in just one app</p>
        <div className="my-11 flex flex-wrap gap-5 ">
          <div className="flex items-center gap-2">
            {Array(5).fill(1).map((_,index)=>(
              <Image src='/star.svg' key={index} alt="star" width={24} height={24} />
            ))}

          </div>
          <p className="bold-16 lg:bold-20 text-blue-70 ">198k </p>
          <span className="regular-16 lg:regular-20 ml-1 capitalize">excellent reviews</span>
        </div>
        <div className="flex w-full gap-3 sm:flex-row flex-col">
         <Button type="button"  
         title="Download app"
          variant="btn_green"/>
         <Button type="button"  
         title="How we work ?"
          variant="btn_white_text"
          icon="/play.svg"
          />
          
        </div>
      </div>
      <div className="relative flex flex-1 items-start ">
      
          <div className="relative z-20 w-[260px] flex-col justify-between rounded-3xl bg-green-90 gap-8  bg-green px-7 py-8">
            <div className="flex flex-col ">
                <div className="flex  items-baseline gap-4 "> 
                   <div>
                   <p className="regular-16 text-gray-20 capitalize">location</p> 
                   <h2 className="regular-20 text-white font-bold">Aguas Calientes</h2>
                   </div>
                
                   <Image src="/close.svg" width={24}  alt="close" height={24} />
                </div>
            </div>
            <div className="flex flex-row mt-8 gap-6">
              <div>
              <p className="regular-16 text-gray-20 capitalize">Distance</p>
               <h2 className="regular-20 font-bold text-white">173.28 mi</h2>
              </div>
              <div>
              <p className="regular-16 text-gray-20 capitalize">Elevation</p>
               <h2 className="regular-20 font-bold text-white">2.040 km</h2>
              </div> 
            </div>
          </div> 
      </div>
    </section>
  );
}

export default HeroSection;
